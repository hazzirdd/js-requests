...
sayHelloButton.addEventListener('mouseout', baseBackground)
// PROBLEM 4
/*
...
const ohMy = () => {
    ...
    axios.get(`${baseURL}/animals`)
        .then((res) => {
        let ohMyArr = res.data;
            for (let i = 0; i < ohMyArr.length; i++) {
                let ohMyTitle = document.createElement('p')
                ohMyTitle.textContent = ohMyArr[i]
                ohMyTitle.appendChild(divTag)
            }
        })
        .catch((err) => {
            console.log(err)
...
const repeatMyParam = () => {
    axios.get(`${baseURL}/repeat/waddup`)
    .then((res) => {
...
// CODE HERE
const queryFunction = () => {
    let checker = `${baseURL}/query-test/?age=one-Hundred`;
    axios.get(`${baseURL}/query-test/?age=one-Hundred`)
    .then((res) => {
        console.log(res.data)
        if (checker.includes('=')) {
            console.log('What happened?')
        } else {checker.includes('=' && '=') }console.log('what happened?')
    })
    .catch((err) => {
        console.log(err)
    })
...
// CODE HERE 
const newFoodText = document.querySelector('#food-input')

const createFood = () => {
    const foodInput = document.querySelector('#food-input');
    
    let newFoodItem = [...newFoodText.value.split("")];
    console.log(newFoodItem);

    body = {
        newFood: newFoodText.value,
    }
...
function createFoodItem(food) {
    ...
    foodItem.innerHTML = `<h3>${food.value}</h3>`

    foodUnList.appendChild(foodItem)
  }